...
# Need to load the DLL's here and not from module manifest (.psd1 - RequiredAssemblies & FileList) because it always load an older version from GAC 

Write-Host "Loading Module Assemblies"

Add-Type -Path "$currentPath\Microsoft.AnalysisServices.Core.dll"
Add-Type -Path "$currentPath\Microsoft.AnalysisServices.Tabular.Json.dll"
Add-Type -Path "$currentPath\Microsoft.AnalysisServices.Tabular.dll"
Add-Type -Path "$currentPath\Newtonsoft.Json.dll"
